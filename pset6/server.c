...
char* indexes(const char* path)
{
    ...
    {
    newPath = malloc(strlen(path) + strlen(htmlIndex) + 1);
    strcpy(newPath, path);
    strcat(newPath, htmlIndex);
    newPath[strlen(path) + strlen(htmlIndex)] = '\0';
    path = newPath;
    free(newPath);
    }
    else if(access("index.php", F_OK) != -1 ) 
    {
    newPath = malloc(strlen(path) + strlen(phpIndex) + 1);
    strcpy(newPath, path);
    strcat(newPath, phpIndex);
    newPath[strlen(path) + strlen(phpIndex)] = '\0';
    path = newPath;
    free(newPath);
    }
    else
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content=malloc(position);
    fread(*content,position,1,file);
    *length=position;
    return true;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char copyLine[strlen(line) + 1];
        ...
        char* copy_absPath;
        char* copy_Query;
        copy_absPath = strtok(requestTarget, "?");
        copy_Query = strtok(NULL, "\0");
        if (copy_Query != NULL)
        {
            strcpy(abs_path, copy_absPath);
            strcpy(query, copy_Query);
     
            abs_path[strlen(copy_absPath)] = '\0';
            query[strlen(copy_Query)] = '\0';
        }
        else
        {
            strcpy(abs_path, copy_absPath);
